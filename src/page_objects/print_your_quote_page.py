...
class PrintYourQuotePage:
    ...
    def save_premium(self, data, output_filename, sheet_name: str):
        ...
        write_excel_cell(f"src/data/output/{output_filename}.xlsx", "Output", row_value=data[sheet_name][0]["Test ID"], column_name="Total Premium", data=total_premium)
        write_excel_cell(f"src/data/output/{output_filename}.xlsx", "Output", row_value=data[sheet_name][0]["Test ID"], column_name="Total Fee", data=total_fee)
        write_excel_cell(f"src/data/output/{output_filename}.xlsx", "Output", row_value=data[sheet_name][0]["Test ID"], column_name="Total Tax", data=total_tax)
        write_excel_cell(f"src/data/output/{output_filename}.xlsx", "Output", row_value=data[sheet_name][0]["Test ID"], column_name="Grand Total", data=grand_total)
