...
class DFApplicantInfoPage:
    ...
    def fill_applicant_info(self, params: DFapplicantInfoParams):
        ...
        self.insured_credit_history_select.select_option(params.insured_credit_history)
        self.check_loading()
        self.any_arson_fraud_conviction_select.select_option(params.Any_Arson_Fraud_convictions)
        self.check_loading()
        self.bankruptcy_in_last_year_select.select_option(params.Bankruptcy_Last_Year)
        self.check_loading()        
        self.foreclosure_in_last_5_years_select.select_option(params.Foreclosure_Last_5Years)
        self.check_loading()
        self.any_past_due_child_support_select.select_option(params.AnyPast_Due_Child_Support)
        self.check_loading()            
        self.repossessions_in_3_years_select.select_option(params.Repossessions_Last_3Years)
        self.check_loading()
        self.prior_insurance_on_this_account_select.select_option(params.Prior_Insuranceon_This_Account)
        self.check_loading()
        if params.Prior_Insuranceon_This_Account == "Yes":

            self.prior_carrier_name_name_input.fill(params.Prior_CarrierName)
            self.check_loading()
            self.prior_insurance_date_input.fill(params.Expiration_Date_of_Prior_Insurance)
            self.check_loading()
            self.prior_policy_number_input.fill(params.Prior_Policy_Number)
            self.check_loading()
            self.prior_insurance_premium_input.fill(params.Prior_Insurance_Premium)
            self.check_loading()
            self.risk_new_to_agency_select.select_option(params.Risk_New_to_Agency)
            self.check_loading()
            self.lapse_30_days_select.select_option(params.Lapse_30_Days)
            self.check_loading()
            self.policy_terminated_at_company_request_select.select_option(params.Terminate_at_Company_Request)
            self.check_loading()
            self.previous_wind_hail_deductible_select.select_option(params.Previous_WindHail_Dedductible)
            self.check_loading()
            self.reason_for_termination_select.select_option(params.Reason_for_Termination)
            self.check_loading()

        expect(self.dwelling_fire_loss_history_btn).to_be_visible()
        expect(self.dwelling_fire_loss_history_btn).to_be_enabled()
